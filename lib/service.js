 ...
 * @param [String] [imagePath] Repository where image is stored
 * @return [Promise]
 */
Service.prototype.registerTaskDefinition = function(version, imagePath) {
  var s = this;
  return this.fetchTaskDefinition(this.service.taskDefinition).then(function(taskDef) {
    return new Promise(function(resolve, reject) {
      // TODO: Revisit the imagePath method of skipping the update for certain container images.
      for (var i = 0; i < taskDef.containerDefinitions.length; i++) {
        // Update the image version number.
        // Maintain original behavior of updating all images if imagePath is not provided
        // Otherwise, only update if the given image repo is the same as what's currently used
        if ((typeof imagePath  === 'undefined') ||
            (imagePath === taskDef.containerDefinitions[i].image.split(':')[0])) {
 ...
 * @return [Promise]
 */
Service.prototype.deploy = function(version, imagePath) {
  var service = this;
  return service.registerTaskDefinition(version, imagePath).then(function(taskArn) {
